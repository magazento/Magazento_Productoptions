 ...
 */
class Magazento_Productoptions_Model_Mysql4_Product_Option extends Mage_Catalog_Model_Resource_Product_Option {
    ...
    protected function _construct() {
        $this->_init('productoptions/product_option', 'option_id');
    }
    ...
    protected function _saveValuePrices(Mage_Core_Model_Abstract $object) {
        ...
        $priceTable = $this->getTable('productoptions/product_option_price');
    ...
    protected function _saveValueTitles(Mage_Core_Model_Abstract $object) {
        $readAdapter = $this->_getReadAdapter();
        $writeAdapter = $this->_getWriteAdapter();
        $titleTable = $this->getTable('productoptions/product_option_title');
    ...
    public function deletePrices($optionId) {
        $this->_getWriteAdapter()->delete(
                $this->getTable('productoptions/product_option_price'), array(
            'option_id = ?' => $optionId
                )
        );
    ...
    public function deleteTitles($optionId) {
        $this->_getWriteAdapter()->delete(
                $this->getTable('productoptions/product_option_title'), array(
            'option_id = ?' => $optionId
                )
        );
    ...
    public function duplicate(Mage_Catalog_Model_Product_Option $object, $oldProductId, $newProductId) {
        ...
        $select = $read->select()
                ->from($this->getTable('productoptions/product_option'))
        ...
        foreach ($optionsCond as $oldOptionId => $newOptionId) {
            // title
            $table = $this->getTable('productoptions/product_option_title');
            ...
            $insertSelect = $write->insertFromSelect(
                    $select, $table, array('option_id', 'store_id', 'title'), Varien_Db_Adapter_Interface::INSERT_ON_DUPLICATE
            );
            $write->query($insertSelect);

            // price
            $table = $this->getTable('productoptions/product_option_price');
    ...
    public function getSearchableData($productId, $storeId) {
        ...
        $titleCheckSql = $adapter->getCheckSql(
                'option_title_store.title IS NULL', 'option_title_default.title', 'option_title_store.title'
        );
        ...
        $defaultOptionJoin = implode(
                ' AND ', array('option_title_default.option_id=product_option.option_id',
        ...
        $select = $adapter->select()
                ...
                ->join(
                        array('option_title_default' => $this->getTable('catalog/product_option_title')), $defaultOptionJoin, array()
                )
                ->joinLeft(
                        array('option_title_store' => $this->getTable('catalog/product_option_title')), $storeOptionJoin, array('title' => $titleCheckSql)
                )
        ...
        $select = $adapter->select()
                ->from(array('product_option' => $this->getMainTable()), null)
                ->join(
                        array('option_type' => $this->getTable('productoptions/product_option_type_value')), 'option_type.option_id=product_option.option_id', array()
                )
                ->join(
                        array('option_title_default' => $this->getTable('productoptions/product_option_type_title')), $defaultOptionJoin, array()
                )
                ->joinLeft(
                        array('option_title_store' => $this->getTable('productoptions/product_option_type_title')), $storeOptionJoin, array('title' => $titleCheckSql)
                )
                ->where('product_option.product_id = ?', $productId);
