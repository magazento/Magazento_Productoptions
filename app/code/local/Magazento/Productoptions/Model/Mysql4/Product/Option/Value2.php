 ...
 */
class Magazento_Productoptions_Model_Mysql4_Product_Option_Value2 extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Option_Value //extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    {
        $this->_init('productoptions/product_option_type_value', 'option_type_id');
    }

   protected function _afterSave(Mage_Core_Model_Abstract $object) {
        $priceTable = $this->getTable('productoptions/product_option_type_price');
        $titleTable = $this->getTable('productoptions/product_option_type_title');
        ...
        if ($object->getStoreId() != '0' && $scope == Mage_Core_Model_Store::PRICE_SCOPE_WEBSITE
            ...
            $this->_getWriteAdapter()->delete(
                    $priceTable, $this->_getWriteAdapter()->quoteInto('option_type_id = ' . $object->getId() . ' AND store_id = ?', $object->getStoreId())
            );
        }
        ...
        if (!$object->getData('scope', 'title')) {
            ...
            if ($this->_getReadAdapter()->fetchOne($statement)) {
                if ($object->getStoreId() == '0') {
                    $this->_getWriteAdapter()->update(
                            $titleTable, array('title' => $object->getTitle()), $this->_getWriteAdapter()->quoteInto('option_type_id=' . $object->getId() . ' AND store_id=?', 0)
                    );
                }
            } else {
                $this->_getWriteAdapter()->insert(
        ...
        if ($object->getStoreId() != '0' && !$object->getData('scope', 'title')) {
            ...
            if ($this->_getReadAdapter()->fetchOne($statement)) {
                $this->_getWriteAdapter()->update(
                        $titleTable, array('title' => $object->getTitle()), $this->_getWriteAdapter()
            ...
            $this->_getWriteAdapter()->delete(
                    $titleTable, $this->_getWriteAdapter()->quoteInto('option_type_id = ' . $object->getId() . ' AND store_id = ?', $object->getStoreId())
            );
        }
    ...
    public function deleteValue($option_id) {
        ...
        $statement = $this->_getReadAdapter()->select()
                ->from($this->getTable('productoptions/product_option_type_value'))
                ->where($condition);
    ...
    public function deleteValues($option_type_id) {
        ...
        $this->_getWriteAdapter()->delete(
                $this->getTable('productoptions/product_option_type_price'), $childCondition
        );
        $this->_getWriteAdapter()->delete(
                $this->getTable('productoptions/product_option_type_title'), $childCondition
        );
    }
    ...
    public function duplicate(Mage_Catalog_Model_Product_Option_Value $object, $oldOptionId, $newOptionId) {
        ...
        foreach ($valueCond as $oldTypeId => $newTypeId) {
            // price
            $table = $this->getTable('productoptions/product_option_type_price');
            ...
            // title
            $table = $this->getTable('productoptions/product_option_type_title');
            $sql = 'REPLACE INTO `' . $table . '` '
